...
export const getResultFromUser = async token => {
  ...
  console.log(`sanity check data.result in fetch-result.js getResultFromUser: ${JSON.stringify(data.result)}`)

  const scores = calculateScore(data.result)
  return { result: getResult({ scores, lang: data.result.lang || 'en' }), user: user }
}
