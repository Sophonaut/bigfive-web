const calculateScore = require('bigfive-calculate-score')
const { getDomain, getFacet } = require('@sophonaut/b5-result-text')
...
export default (data, language) => {
  ...
  Object.keys(repacked).forEach(domainKey => {
    results.push({
      ...
      percentile: repacked[domainKey].percentile,
      average: repacked[domainKey].percentile,
